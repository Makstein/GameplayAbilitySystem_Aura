                                              ...
                                              FActiveGameplayEffectHandle ActiveGameplayEffectHandle)
{
	...
	if (AssetTags.HasTagExact(CooldownTag) || GrantedTags.HasTagExact(CooldownTag))
	{
		...
		if (auto TimeRemainingArray = ASC->GetActiveEffectsTimeRemaining(GameplayEffectQuery); TimeRemainingArray.Num()
			> 0)
		{
			auto TimeRemaining = TimeRemainingArray[0];
			for (auto i = 0; i < TimeRemainingArray.Num(); i++)
			{
