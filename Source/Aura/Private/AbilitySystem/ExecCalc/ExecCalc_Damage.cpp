	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const auto& Pair : FAuraGameplayTags::Get().DamageTypesToResistance)
	{
		...
		checkf(AuraDamageStatics().TagsToCaptureDefs.Contains(ResistanceTag), TEXT("TagToCaptureDefs doesn't contains Tag: [%s] in ExecCalc_Damage"), *ResistanceTag.ToString());
	...
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(DamageStatics().CriticalChanceDef, EvaluateParams, CriticalChance);
	CriticalChance = FMath::Max<float>(0.f, CriticalChance);

	float CriticalResistance = 0.f;
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(DamageStatics().CriticalResistanceDef, EvaluateParams, CriticalResistance);
	...
	const float CriticalResistanceCoefficient = CriticalResistanceCurve->Eval(TargetCombatInterface->GetPlayerLevel());

	float CriticalDamage = 0.f;
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(DamageStatics().CriticalDamageDef, EvaluateParams, CriticalDamage);
